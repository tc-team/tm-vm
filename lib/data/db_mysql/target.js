...
exports.getTarget = function(id, callback) {
	var sql  = 'SELECT * FROM `target` WHERE `id` ="' + id + '"';
...
exports.setTargetCoordinates = function(newLongitude, newLatitude, id, callback) {
	var sql = 'UPDATE `target` SET `longitude` ="' + newLongitude + '", `latitude` ="' + newLatitude + '" WHERE `id` ="' + id + '"';
	connection.query(sql, function(error, result) {
		if (error) {
			callback(error, null);
		} else {
