...
exports.addTarget = function(title, longtitude, latitude, description, status, username, callback) {
	var sql = 'INSERT INTO `target` (`title`, `longtitude`, `latitude`, `description`, `status`, `username`) \
	VALUES ("' + title + '", "' + longtitude + '", "' + latitude + '", "' + description + '","' + status + '","' + username + '")';
...
exports.getAllTargets = function(username, callback) {
	var status = true;
	var sql = 'SELECT * FROM `target` WHERE `username` ="' + username + '"';
	connection.query(sql, function(error, rows) {
		if (error) {
			callback(error, null);
		} else {
