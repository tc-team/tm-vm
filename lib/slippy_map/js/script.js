...
function updateCoordsArray(rows, columns, tileX, tileY, i, j) {
...
window.onload = function () {
	...
	var mapOffsetX = -(((columns * divSize) - mapWidth) / 2);
	var mapOffsetY = -(((rows * divSize) - mapHeight) / 2 );
	var h = mapOffsetY;
	var w = mapOffsetX;
	...
	var defaultOffsetX = (tileX - centerMapX) * divSize;
	var defaultOffsetY = (tileY - centerMapY) * divSize;
	...
	var allowableOffsetTop = divSize + defaultOffsetY;
	var allowableOffsetLeft = divSize + defaultOffsetX;
	var allowableOffsetRight = divSize - defaultOffsetX;
	var allowableOffsetBottom = divSize - defaultOffsetY;
	...
	marker.style.top = (tileY * divSize - Math.abs(mapOffsetY) - coordsArray[0][0].y * divSize) + 'px';
	marker.style.left = (tileX * divSize - Math.abs(mapOffsetX) - coordsArray[0][0].x * divSize) + 'px';
	...
	function changeScale(oldZoom, mouseX, mouseY) {
		...
		tileY = lat2tile(mouseLat, zoom);
		tileX = long2tile(mouseLong, zoom);

		xTotal = Math.pow(2, zoom);
		var i = mouseX2I(tileXzoomed);
		var j = mouseY2J(tileYzoomed);
		coordsArray = updateCoordsArray(rows, columns, tileX, tileY, i, j);
	...
	function mouseX2TileX (mouseX) {
		...
		return (pointX / divSize + coordsArray[0][0].x);
	}

	function mouseY2TileY (mouseY) {
		var pointY = Math.abs(mapOffsetY) + mouseY;
		return (pointY / divSize + coordsArray[0][0].y);
	}
