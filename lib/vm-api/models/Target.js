...
var mysql = require('../../common/db_clients/mysqlHelper.js');
...
var Target = {
	...
	add: function(title, longtitude, latitude, description, status, username, callback) {
		...
		mysqlsql.queryMysql(sql, function(error, result) {
			if (error) {
				callback(error, null);
			} else {
	...
	set: function(title, description, status, longtitude, latitude, username, callback) {
		var sql = 'UPDATE `target` SET `title` ="' + title + '", `description` ="' + description + '", 
			`status` ="' + status + '" WHERE `longtitude` ="' + longtitude + '" AND `latitude` ="'
