			...
			function(error, result) {
				if (error) {
					...
					if (req.body.newTitle) {
						targetService.setTarget(req.body.newTitle, result[0].description,
							result[0].status, req.body.longtitude, req.body.latitude, 
					...
					if (req.body.newDescription) {
						targetService.setTarget(result[0].title, req.body.newDescription, 
							result[0].status, req.body.longtitude, req.body.latitude,
							req.cookies.username, function(error, result) {
								if (error) {
									res.status(500).send(error);
					...
					if (req.body.newStatus) {
						targetService.setTarget(result[0].title, result[0].description,
