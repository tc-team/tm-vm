...
var TargetRouter = {
	...
	readAllTargets: function(req, res) {
		targetService.getAllTargets(username, function(error, result) {
			if (error) {
				res.status(500).send(error);
			} else {
				res.status(200).send(result);
	...
	updateTarget: function(req, res) {
		if (req.body.newTitle) {
			targetService.setTargetTitle(req.body.newTitle, req.params.longtitude, req.params.latitude,
		...
		if (req.body.newDescription) {
			targetService.setTargetDescription(req.body.newDescription, req.params.longtitude,
		...
		if (req.body.newStatus) {
			targetService.setTargetStatus(req.body.newStatus, req.params.longtitude, req.params.latitude,
