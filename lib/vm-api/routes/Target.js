...
var TargetRouter = {
		...
		targetService.addTarget(req.body.title, req.body.longitude, req.body.latitude,
			req.body.description, req.body.status, req.cookies.username, function(error, result) {
				if (error) {
					...
					res.status(200).send('Target added');
				}
			});
	},

	readTarget: function(req, res) {
		targetService.getTarget(req.params.id, function(error, result) {
			if (error) {
				res.status(500).send(error);
			} else {
	...
	updateTarget: function(req, res) {
		if (req.body.newTitle) {
			...
			targetService.setTargetCoordinates(longitude, latitude, id, function(error, result) {
				if (error) {
					res.status(500).send(error);
				} else {
					res.status(201).send('You changed target coordinates');
				}
			});
