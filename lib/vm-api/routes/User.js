...
var UserRouter = {
	...
	updateUser: function (req, res) {
		if (req.body.newPassword) {
			...
			userService.setUser(req.cookies.username, hash, req.session.email, function (error, result) {
				if (error) { 
					res.status(500).send(error);
					return;
				} else {
			...
			userService.setUser(req.cookies.username, req.session.password, req.body.newEmail, function (error, result) {
				if (error) { 
					res.status(500).send(error);
					return;
				} else {
