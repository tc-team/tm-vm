...
var SlippyMap = function (map, options) {
	...
	var tile_scaled_container = document.createElement('DIV');
	//tile_scaled_container.setAttribute('id', 'tile_scaled_container');
	map.appendChild(tile_scaled_container);
	...
	/*	tile_container.style.top = tile_container.offsetTop + evt.movementY + 'px';
		tile_container.style.left = tile_container.offsetLeft + evt.movementX + 'px';*/
	...
	map.addEventListener('wheel', function(evt) {
		...
		if (oldZoom != zoomLevel) {
			changeZoom(oldZoom, evt.pageX - this.offsetLeft, evt.pageY - this.offsetTop);
		}
	});
	...
	map.addEventListener("transitionend", function () {
		loadMap(zoomLevel, zoomOffsetX, zoomOffsetY, function() {
			map.removeChild(tile_scaled_container);
			
			tile_container.removeAttribute('class');
			for(var i = 0, n = tilesNodes.length; i < n; i++) {
				tilesNodes.item(i).firstElementChild.removeAttribute('class');
			};
			draw = false;
		});
	});
	...
	function updateScaledContainer () {
		tile_scaled_container = tile_container.cloneNode(true);
		map.appendChild(tile_scaled_container);
	}
	...
	function changeZoom(oldZoom, mouseX, mouseY) {
		...
		tile_scaled_container.style.transition = 'transform .2s linear';
		...
		if (oldZoom < zoomLevel) {			
			...
			tile_scaled_container.style.transform = 'matrix('+ scale +', 0, 0, '+ scale +',' + translateX + ', '+ translateY +')';
		} else {
			translateX = mouseXinPX / scale + centermapOffsetX;
			translateY = mouseYinPX / scale + centermapOffsetY;
			tile_scaled_container.style.transform = 'matrix('+ 1/scale +', 0, 0, '+ 1/scale +',' + translateX + ', '+ translateY +')';
		}		
		...
		defaultOffsetY = offsetY - mapOffsetY;
		defaultOffsetX = offsetX - mapOffsetX;
		...
		tile_container.setAttribute('class', 'visible-img');
		tile_container.style.top = mapOffsetY + 'px';
		tile_container.style.left = mapOffsetX  + 'px';
	}

	
	loadMapStart(zoomLevel, zoomOffsetX, zoomOffsetY, function(msg) {
	...
	function loadMapStart (zoom, zoomOffsetX, zoomOffsetY, callback) {
