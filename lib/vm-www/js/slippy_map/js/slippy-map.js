...
var SlippyMap = function (map, options) {
	...
	function changeZoom(oldZoom, mouseX, mouseY) {
		...
		if (oldZoom < zoomLevel) {
			var scale = 2;
			var translateX = - Math.floor(columns * tileSize / 2);
			var translateY = - Math.floor(rows * tileSize / 2);	
		} else {
			var scale = 0.5;
			var translateX = Math.floor(columns * tileSize / 4);
			var translateY = Math.floor(rows * tileSize / 4);	
		}
		tile_container.style.transition = 'transform 1s linear';
		...
		loadMap(zoomLevel, function(){
			tile_container.style.transition = '';
			tile_container.style.transform = '';
			tile_container.style.top = mapOffsetY + 'px';
			tile_container.style.left = mapOffsetX + 'px';
		});
		var markerOffsetX = tileX * tileSize - Math.abs(mapOffsetX) - coordsArray[0][0].x * tileSize - centermapOffsetX; 
		var markerOffsetY = tileY * tileSize - Math.abs(mapOffsetY) - coordsArray[0][0].y * tileSize - centermapOffsetY;
	...
	loadMap(zoomLevel, function(msg) {
		
	});

	function loadMap (zoom, callback) {
		var count = 0;
	...
	/*function loadMap (zoom) {
		...
		worker.onmessage = function (e) {
	    	tilesNodes.item(e.data.tileNumber).firstElementChild.src = url + e.data.dir + e.data.zoom + '-' +e.data.x + '-' + e.data.y + '.png';
		};
	}*/
	...
	function mouseX2TileX (mouseX) {
		var pointX = Math.abs(tile_container.offsetLeft) + mouseX;
		return (pointX / tileSize + coordsArray[0][0].x);
	}

	function mouseY2TileY (mouseY) {
		var pointY = Math.abs(tile_container.offsetTop) + mouseY;
